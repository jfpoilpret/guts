...
public class Bean<T>
{
	...
	{
		Bean<T> helper = (Bean<T>) _cache.get(clazz);
		if (helper == null)
		{
			helper = new Bean<T>(clazz);
	...
	// The returned reference can be used to get/set property value or get its name (in a safe way)
	public<U> PropertyImpl<T, U> property(U mockCall)
	{
	...
	// Create a new bean that delegates to this one but makes all its properties bound
	...
	public T proxy(T source)
	{
		...
		enhancer.setInterfaces(
			new Class[]{ChangeListenerAdapter.class, ProxySource.class});
		enhancer.setCallback(new ProxyInterceptor<T>(source, _properties));
		return _clazz.cast(enhancer.create());
	}
	
	@SuppressWarnings("unchecked")
	public T source(T proxy)
	{
	...
	private void checkType(Object property, PropertyDescriptor descriptor)
	{
		...
		//TODO pending cases: should make sure mock getters always return non null!
	...
	// Global cache of ObjectHelper objects (each class T should have only one ObjectHelper<T> instance)
