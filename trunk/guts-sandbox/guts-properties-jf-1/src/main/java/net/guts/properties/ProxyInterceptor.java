...
class ProxyInterceptor<T> implements MethodInterceptor
{
		...
		throws Throwable
	{
		...
		for (PropertyDescriptor descriptor: _properties)
		{
			...
			{
				// If there's a getter, read _source old value, else consider it null
	...
	private static final Map<Signature, Method> _pclMethods =
		new HashMap<Signature, Method>();
	...
	static
	{
		try
		{
			_proxySourceMethod = ReflectUtils.getSignature(
				ProxySource.class.getMethod("source"));
		}
		catch (Exception e)
		{
			// Should never happen normally!
